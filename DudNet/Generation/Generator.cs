...
internal static class Generator
{
	...
	)
	{
		...
		stringBuilder
			...
			.IndentedBlockWrite(sb =>
			{
				...
				{
					sb.AppendLine($"return ({methodSymbol.ReturnType}) default;");
				}
			})
			.AppendLine('}');
	}
	...
	)
	{
		foreach (var method in methodSymbols)
		{
			...
			stringBuilder.AppendLine($"partial void {name}Interceptor({parameters});");
		}
	}
	...
	/// <param name="skipPropertyMethods">Whether or not auto-property methods should be skipped.</param>
	...
	)
	{
		foreach (var method in methodSymbols)
		{
			...
			if (isPropertyMethod && skipPropertyMethods)
			{
				continue;
			}
	...
	private static string GenerateMethodDeclaration(IMethodSymbol methodSymbol)
	{
		...
		{
			result.Append($"<{GetTypeParameterAsString(methodSymbol.TypeParameters)}>");
		}
...
#pragma warning restore RS1024
		...
		{
			if (!TryGetAutoProperty(method, out var property))
			{
				continue;
			}
...
#pragma warning restore RS1024
	...
	private static bool TryGetAutoProperty(IMethodSymbol methodSymbol, out IPropertySymbol? propertySymbol)
	{
		...
		if (methodSymbol.MethodKind is not (MethodKind.PropertyGet or MethodKind.PropertySet))
		{
			return false;
		}

		if (methodSymbol.AssociatedSymbol is not IPropertySymbol property)
		{
			return false;
		}
